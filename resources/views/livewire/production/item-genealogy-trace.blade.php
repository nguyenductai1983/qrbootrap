<div class="container-fluid py-4">
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-primary text-white py-3 d-flex justify-content-between align-items-center">
            ...
            <a href="{{ route('items') }}" class="btn btn-sm btn-light fw-bold text-primary">
                ...
                <li class="genealogy-item is-root">
                    <div class="card genealogy-card is-root p-3">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <div>
                                <span class="badge bg-primary mb-1"><i class="fa-solid fa-crosshairs me-1"></i> Gốc hiện
                                    tại</span>
                            ...
                            <div class="text-end">
                                ...
                                <div class="small text-muted mt-1"><i class="fa-regular fa-clock me-1"></i>
                                    {{ $rootItem->created_at->format('d/m/Y H:i') }}</div>
                            </div>
                        </div>
                        <div class="row g-2 small">
                            <div class="col-md-3 col-6"><strong>Sản phẩm:</strong> {{ $rootItem->product->name ?? '-' }}
                            </div>
                            <div class="col-md-3 col-6"><strong>Dài Gốc:</strong>
                                {{ (float) $rootItem->original_length }}m</div>
                            <div class="col-md-3 col-6"><strong>Dài Còn:</strong> <span
                                    class="text-danger fw-bold">{{ (float) $rootItem->length }}m</span></div>
                            <div class="col-md-3 col-6"><strong>Khối lượng:</strong> {{ (float) $rootItem->weight }}kg
                            </div>

                            <div class="col-md-3 col-6"><strong>Xưởng:</strong> {{ $rootItem->department->name ?? '-' }}
                            </div>
                            <div class="col-md-3 col-6"><strong>Máy:</strong> {{ $rootItem->machine->name ?? '-' }}
                            </div>
                            <div class="col-md-3 col-6"><strong>Người tạo:</strong>
                                {{ $rootItem->creator->name ?? '-' }}</div>
                            ...
                            @if (is_array($rootItem->properties))
                                @if (isset($rootItem->properties['LAMI']) && $rootItem->properties['LAMI'] > 0)
                                    <div class="col-12 mt-2"><span class="badge bg-dark">Lami:
                                            {{ $rootItem->properties['LAMI'] }}</span></div>
                                @endif
                            @endif
                        </div>
                        <div class="mt-3 text-end">
                            <button wire:click="reprintItems([{{ $rootItem->id }}])"
                                class="btn btn-sm btn-outline-primary"><i class="fa-solid fa-print me-1"></i> In
                                Tem</button>
                        </div>
                    </div>
                </li>
